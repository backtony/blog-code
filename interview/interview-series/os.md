...
## 운영체제
![그림1](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/os/os-1.PNG?raw=true)  
...
## 메모리구조
![그림2](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/os/os-2.PNG?raw=true)  
...
+ 힙 영역
    ...
    - __런타임 시 크기__ 가 결정된다.
    - __자바에서는 가비지 컬렉터가 메모리 공간을 자동으로 해제__ 해준다.
...
__진행 과정을 묻는게 면접질문으로 받았었으니 잘 기억하자.__
...
### 교착 상태 해결 방법
위 4가지 조건 중 하나라도 제거하면 교착 상태를 막을 수 있다.  
...
## 전통적인 동기화 예제
운영체제에서 동기화란 병렬적으로 수행되는 작업들에 대해 __자원 접근 순서를 정해 서로가 알고있는 정보가 일치하도록 하는 것__
...
### 유한한 공유 버퍼 문제 (Bounded - Buffer Problem)
여기서 프로세스는 두 가지 종류가 있다. 하나는 Producer(생산자) 프로세스이고, 또 다른 하나는 Consumer(소비자) 프로세스이다.  
이러한 프로세스들끼리 공유하는 버퍼가 존재하는데, 생산자 프로세스는 데이터를 만들어서 버퍼에다 집어넣어주고, 소비자 프로세스는 여기엥서 데이터를 꺼내가는 역할을 한다.  
어떤 생산자 프로세스가 버퍼 중에 빈 곳에다가 데이터를 쓰려고 한다고 하자. 그러는 와중에 CPU를 뺏기고 CPU를 얻은 다른 생산자 프로세스도가 해당하는 빈 곳에다가 데이터를 쓴다고 해보자. 그러면 둘 중에 한 프로세스의 데이터는 유실이 될 수 있다는 것이다.   

### 읽기 쓰기 문제 (Readers - Writers Problem)
주로 공통된 데이터베이스를 접근할 때 발생하는 문제이다. 데이터베이스는 사용자들이 모두 공통으로 사용하므로 한 번에 한 개의 프로세스만 접근하도록 처리했으나 이는 비효율적이다. 따라서 아래와 같이 해결한다.
+ reader가 임계영역에 들어갈 경우, 다른 reader는 접근 가능, writer는 접근 불가
+ writer가 임계영역에 들어갈 경우, reader와 writer 모두 접근 불가
...
초기버전에서는 Critical Section에 진입해야하는 프로세스는 진입 코드를 계속 반복실행하여 CPU시간을 낭비했었는데 이것을 __Busy Waiting__ 이라고 한다.  
현재는 일반적으로는 Critical Section에 진입을 시도했지만 실패한 프로세스는 Block 시킨 뒤, Critical Section에 자리가 날 때 다시 깨우는 방식으로 사용하고 있다.  
...
+ 커널 작업을 수행하는 중에 인터럽트 발생
    - 문제 : 커널모드에서 데이터를 로드하여 작업을 수행하다가 인터럽트가 발생하여 같은 데이터를 조작하는 경우
    - 해결 : 커널모드에서 작업을 수행하는 동안, 인터럽트를 disable 시켜 CPU 제어권을 가져가지 못하게 한다.
+ 프로세스가 System Call을 하여 커널 모드로 진입하여 작업을 수행하는 도중 문맥 교환이 발생할 때
    - 문제 : 프로세스1이 커널모드에서 데이터를 조작하는 도중 시간 초과되어 CPU 제어권이 다른 프로세스2로 넘어가 같은 데이터를 조작하는 경우(프로세스2가 작업에 반영되지 않음)
    - 해결 : 프로세스가 커널모드에서 작업하는 경우 시간이 초과되어도 CPU 제어권이 다른 프로세스에게 넘어가지 않도록 함
+ 멀티 프로세서 환경에서 공유 메모리 내의 커널 데이터에 접근할 떄
    - 문제 : 멀티 프로세서 환경에서 2개의 CPU가 동시에 커널 내부의 공유 데이터에 접근하여 조작하는 경우
    - 해결 : 커널 내부에 있는 각 공유 데이터에 접근할 때마다, 그 데이터에 대한 lock/unlock을 설정하는 방법

### 해결법
+ 상호 배제
+ 동기화
    - 세마포어
    - 모니터 : 세마포어는 오래된 동기화 도구로 현재는 모니터를 사용한다. 개발자가 Counter 값을 올리고 내리는 고민 필요 없이 개발자는 키워드를 통해 편하게 동기화할 수 있다.
    - 락
...
+ Priority Scheduling
    - 우선순위 순서대로 할당
    - __기아 상태__ 발생(우선순위가 낮아서 실행될 수가 없음)
...
### 메모리 관리 전략
#### 페이징
![그림3](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/os/os-3.PNG?raw=true)  
...
#### 세그멘테이션
![그림4](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/os/os-4.PNG?raw=true)  
...
### 이점
![그림5](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/os/os-5.PNG?raw=true)  
