...
## DB를 사용하는 이유
+ 파일 시스템의 __데이터 중복, 비일관성, 검색 등의 문제__ 를 해결하기 위해
...
## 열
+ 테이블을 구성하는 데이터들 중 세로로 묶은 데이터 셋으로 __속성__ 이라고 부름
...
## 키
![그림1](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/db/db-1.PNG?raw=true)  
...
+ 낙관적락 옵션
    - None
        ...
        - 동작 : 엔티티를 수정할 때 버전을 체크하면서 버즌을 증가시킨다. 만약 버전값이 현재 버전이 아니면 예외가 발생한다.
...
반면에 MVCC는 레코드에 잠금을 걸지 않고 트랜잭션 격리 레벨에 따라 __일관된 읽기__ 를 제공한다.  
...
### 자료구조
#### B 트리
![그림2](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/db/db-2.PNG?raw=true)  
+ 이진 트리를 확장해서 많은 자식을 갖을 수 있는 __균형 트리__
+ key들이 항상 오름차순으로 정렬되어 구성
+ Branch와 Leaf 노드가 key와 data를 저장

#### B+ 트리
![그림3](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/db/db-3.PNG?raw=true)  
...
## 함수의 종속성
X -> Y : X값을 알면 Y를 알 수 있고, X값에 의해 Y값이 달라지면 Y는 X에 함수적 종속이다.
...
## 클러스터링
![그림4](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/db/db-4.PNG?raw=true)  
...
## 레플리케이션
![그림5](https://github.com/backtony/blog-code/blob/master/interview/interview-series/img/db/db-5.PNG?raw=true)  
...
## 행의 개수가 많은 테이블 설계
__실제 면접에서 받은 질문이다. 꼬리 질문으로 쿼리를 최적화하는 방법까지 물어봤다.__
